...
class TestRectangularBinning:
    ...
    def test_construct_from_array_edges(self):
        ...
        config = {"bin_vars_edges": [("energy", "array", [1.0, 2.0, 5.0, 10.0])]}
        binning = RectangularBinning.construct_from(config)
    ...
    def test_construct_from_unknown_bin_type(self):
        ...
        config = {"bin_vars_edges": [("energy", "unknown_type", [1.0, 10.0, 5])]}
    ...
    def test_calculate_bin_indices_single_variable(self):
        ...
        np.testing.assert_array_equal(
            binning.bin_indices_dict["test_dataset"][0], expected_indices
        )
    ...
    def test_calculate_bin_indices_multiple_variables(self):
        ...
        np.testing.assert_array_equal(
            binning.bin_indices_dict["test_dataset"][0], expected_energy_indices
        )
        np.testing.assert_array_equal(
            binning.bin_indices_dict["test_dataset"][1], expected_angle_indices
        )
    ...
    def test_calculate_bin_indices_mismatched_lengths(self):
        ...
        with pytest.raises(
            ValueError, match="All binning variables must have the same length"
        ):
            binning.calculate_bin_indices("test_dataset", binning_variables)
    ...
    def test_build_histogram_with_masking(self):
        ...
        weights = backend.array(
            [10.0, 1.0, 2.0, 20.0]
        )  # Out-of-bounds weights should be ignored
